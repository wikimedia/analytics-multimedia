...
SELECT * FROM (
    SELECT
        ...
        SUBSTRING_INDEX(SUBSTRING_INDEX(GROUP_CONCAT(event_domComplete
            ORDER BY event_domComplete SEPARATOR ','), ',', 50/100*COUNT(*)+1), ',', -1) AS filepage_loggedin_50,
        SUBSTRING_INDEX(SUBSTRING_INDEX(GROUP_CONCAT(event_domComplete
            ORDER BY event_domComplete SEPARATOR ','), ',', 90/100*COUNT(*)+1), ',', -1) AS filepage_loggedin_90,
        SUBSTRING_INDEX(SUBSTRING_INDEX(GROUP_CONCAT(event_domComplete
            ORDER BY event_domComplete SEPARATOR ','), ',', 95/100*COUNT(*)+1), ',', -1) AS filepage_loggedin_95,
        SUBSTRING_INDEX(SUBSTRING_INDEX(GROUP_CONCAT(event_domComplete
    ...
    SELECT
        DATE_FORMAT(timestamp, '%Y-%m-%d') AS datestring,
        SUBSTRING_INDEX(SUBSTRING_INDEX(GROUP_CONCAT(event_domComplete
            ORDER BY event_domComplete SEPARATOR ','), ',', 50/100*COUNT(*)+1), ',', -1) AS filepage_anon_50,
        SUBSTRING_INDEX(SUBSTRING_INDEX(GROUP_CONCAT(event_domComplete
            ORDER BY event_domComplete SEPARATOR ','), ',', 90/100*COUNT(*)+1), ',', -1) AS filepage_anon_90,
        SUBSTRING_INDEX(SUBSTRING_INDEX(GROUP_CONCAT(event_domComplete
            ORDER BY event_domComplete SEPARATOR ','), ',', 95/100*COUNT(*)+1), ',', -1) AS filepage_anon_95,
        SUBSTRING_INDEX(SUBSTRING_INDEX(GROUP_CONCAT(event_domComplete
