...
) MultimediaViewerNetworkPerformanceUnioned WHERE wiki = 'mediawikiwiki' AND event_type != 'image' AND timestamp < TIMESTAMP(CURDATE()) AND timestamp >= TIMESTAMP(DATE_SUB(CURDATE(), INTERVAL 10 DAY)) AND event_total > 20 AND event_country != '' GROUP BY country HAVING sample_size > 10 ORDER BY mean ASC