...
) AS MultimediaViewerNetworkPerformanceUnioned WHERE wiki = 'czwiki' AND timestamp >= TIMESTAMP(DATE_SUB(NOW(), INTERVAL 30 DAY)) AND event_type = 'image' GROUP BY datestring ORDER BY datestring ASC ) stats USING (datestring)