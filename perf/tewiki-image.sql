...
) AS MultimediaViewerNetworkPerformanceUnioned WHERE wiki = 'tewiki' AND timestamp < TIMESTAMP(CURDATE()) AND timestamp >= TIMESTAMP(DATE_SUB(CURDATE(), INTERVAL 30 DAY)) AND event_total > 20 AND event_type = 'image' GROUP BY datestring ORDER BY datestring ASC ) stats USING (datestring)