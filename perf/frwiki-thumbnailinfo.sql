...
) AS MultimediaViewerNetworkPerformanceUnioned WHERE wiki = 'frwiki' AND timestamp >= TIMESTAMP(DATE_SUB(NOW(), INTERVAL 30 DAY)) AND event_type = 'thumbnailinfo' GROUP BY datestring ORDER BY datestring ASC ) stats USING (datestring)