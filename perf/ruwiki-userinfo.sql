...
) AS MultimediaViewerNetworkPerformanceUnioned WHERE wiki = 'ruwiki' AND timestamp >= TIMESTAMP(DATE_SUB(NOW(), INTERVAL 30 DAY)) AND event_type = 'userinfo' GROUP BY datestring ORDER BY datestring ASC ) stats USING (datestring)