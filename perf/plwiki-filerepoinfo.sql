...
) AS MultimediaViewerNetworkPerformanceUnioned WHERE wiki = 'plwiki' AND timestamp < TIMESTAMP(CURDATE()) AND timestamp >= TIMESTAMP(DATE_SUB(CURDATE(), INTERVAL 30 DAY)) AND event_type = 'filerepoinfo' GROUP BY datestring ORDER BY datestring ASC ) stats USING (datestring)