...
) AS MultimediaViewerNetworkPerformanceUnioned WHERE wiki = 'dewiki' AND timestamp < TIMESTAMP(CURDATE()) AND timestamp >= TIMESTAMP(DATE_SUB(CURDATE(), INTERVAL 30 DAY)) AND event_type = 'globalusage' GROUP BY datestring ORDER BY datestring ASC ) stats USING (datestring)